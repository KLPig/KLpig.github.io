...
logincode.addEventListener(
    'input',
    function(){
        if(logincode.value == 'secret' || logincode.value == 'document' || logincode.value == 'KunGame'){
            loginElement = document.getElementsByClassName('login-element-toggle')
            for(var i = 3; i >= 0; i--){
                loginElement[i].className = 'login-element-leave';
            }
...
password.addEventListener(
    ...
    function(){
        ...
        if((usrname.value == 'visitor213' && password.value == '1145114' + s) || 
        (usrname.value == 'klpig' && password.value == 'KLpig' + s)){
            loginElement = document.getElementsByClassName('login-element-toggle')
            for(var i = 3; i >= 0; i--){
                loginElement[i].className = 'login-element-leave';
            }
            ...
            setTimeout(
                ...
                function(){
                    ...
                    var nt = Number(date.getFullYear()) * 7 + Number(date.getMonth()) * 11 + Number(date.getDay())
                    var s = (nt % 901 * (nt % 901) * 114 % 901 + 191) * 810 % 901 + 100
                    if(usrname.value == 'klpig')
                        alert('Today\'s password: ' + s)
